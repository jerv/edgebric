...
export function getFeedbackStats(since?: string): { up: number; down: number; total: number } {
  const db = getDb();
...
export function getTopicClusters(minCount: number = 5): Array<{ topic: string; count: number; upRate: number }> {
  const db = getDb();
...
export function listFeedback(limit: number = 100): Feedback[] {
  const db = getDb();
  const rows = db
    .select()
    .from(feedback)
