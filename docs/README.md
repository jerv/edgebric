# Edgebric — Project Documentation

**Name:** Edgebric
**Status:** Active development — v0.5 beta
**Last Updated:** 2026-03-29

---

## Documents

| File | Contents | Status |
|---|---|---|
| [01-overview.md](01-overview.md) | Vision, problem statement, positioning, differentiation | CURRENT |
| [02-users.md](02-users.md) | User personas, user stories | CURRENT |
| [03-features.md](03-features.md) | Feature requirements (MVP / V2 / Future) | CURRENT |
| [04-technical.md](04-technical.md) | Architecture, ingestion pipeline, tech stack | MOSTLY CURRENT |
| [05-decisions.md](05-decisions.md) | Resolved design decisions and open questions | CURRENT |
| [06-metrics.md](06-metrics.md) | Success metrics | ASPIRATIONAL |
| [07-dev-plan.md](07-dev-plan.md) | Development plan — phases, schema | CURRENT |
| [08-productization.md](08-productization.md) | Phase 2 productization details | CURRENT |
| [09-demo-plan.md](09-demo-plan.md) | Demo plan | ASPIRATIONAL |
| [10-architecture-onepager.md](10-architecture-onepager.md) | Architecture one-pager (compliance audience) | CURRENT |
| [10-pipeline.md](10-pipeline.md) | Branch strategy, CI/CD pipeline | CURRENT |
| [11-pricing-distribution.md](11-pricing-distribution.md) | AGPL 3.0 license, pay-what-you-want distribution | CURRENT |
| [12-desktop-app-plan.md](12-desktop-app-plan.md) | Desktop GUI app (Electron) — current state | CURRENT |

---

## Quick Summary

Edgebric is a privacy-first, on-premise AI knowledge platform that answers questions from a company's own policy and knowledge documents — running entirely on company hardware using llama.cpp for local AI inference and sqlite-vec for vector search. Zero data leaves the building.

**Platform:** Any sensitive knowledge domain — HR, legal, compliance, finance, IT, and beyond.

> *"Your data. Your hardware. Your AI."*
